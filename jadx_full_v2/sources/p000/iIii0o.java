            package p000;

            import com.google.mlkit.vision.barcode.common.Barcode;
            import java.io.IOException;
            import java.io.OutputStream;
            import java.lang.invoke.VarHandle;
            import java.lang.reflect.InvocationTargetException;
            import java.nio.ByteBuffer;
            import java.nio.ByteOrder;
            import java.nio.charset.Charset;
            import java.util.Collection;
            import java.util.HashMap;
            import java.util.Iterator;
            import java.util.Map;
            
/* 4 */     public final class iIii0o implements OIOi0o {
                public static final Charset I0001Ioi1lo = Charset.forName("UTF-8");
                public static final IlIO00o1 I000II;
                public static final IlIO00o1 I000O01llI0;
                public static final iIOo0oiIi1 I000OOo1O;
                public OutputStream I00000oIO;
                public HashMap I00000oOI;
                public HashMap I0000Il00O;
                public OIOi0lOI I0000O;
                public OOiI1O I0000oI00;

                static {
/* 11 */            OlOO1i11110 olOO1i11110I00000oIO = IlIO00o1.I00000oIO("key");
/* 17 */            i1liOIOoi i1lioiooi = new i1liOIOoi();
/* 21 */            i1lioiooi.I0000O = 1;
/* 23 */            VarHandle.storeStoreFence();
/* 26 */            olOO1i11110I00000oIO.I001i1lo1io(i1lioiooi);
/* 33 */            I000II = olOO1i11110I00000oIO.I000O01llI0();
/* 37 */            OlOO1i11110 olOO1i11110I00000oIO2 = IlIO00o1.I00000oIO("value");
/* 43 */            i1liOIOoi i1lioiooi2 = new i1liOIOoi();
/* 47 */            i1lioiooi2.I0000O = 2;
/* 49 */            VarHandle.storeStoreFence();
/* 52 */            olOO1i11110I00000oIO2.I001i1lo1io(i1lioiooi2);
/* 59 */            I000O01llI0 = olOO1i11110I00000oIO2.I000O01llI0();
/* 63 */            I000OOo1O = iIOo0oiIi1.I00000oOI;
                }

                public static int I0000O(IlIO00o1 ilIO00o1) {
/* 7 */             iIIoOIOO0Io iiiooioo0io = (iIIoOIOO0Io) ilIO00o1.I00000oOI(iIIoOIOO0Io.class);
/* 9 */             if (iiiooioo0io != null) {
/* 11 */                return iiiooioo0io.zza();
                    }
/* 18 */            iilo0lOo110.I0000O("Field has no @Protobuf config");
/* 21 */            return 0;
                }

                public final OIOi0o I00000oIO(IlIO00o1 ilIO00o1, Object obj, boolean z) throws IllegalAccessException, IOException, IllegalArgumentException, InvocationTargetException {
/* 1 */             if (obj != null) {
/* 7 */                 if (obj instanceof CharSequence) {
/* 9 */                     CharSequence charSequence = (CharSequence) obj;
/* 11 */                    if (!z || charSequence.length() != 0) {
/* 29 */                        I0001Ioi1lo((I0000O(ilIO00o1) << 3) | 2);
/* 38 */                        byte[] bytes = charSequence.toString().getBytes(I0001Ioi1lo);
/* 43 */                        I0001Ioi1lo(bytes.length);
/* 48 */                        this.I00000oIO.write(bytes);
/* 51 */                        return this;
                            }
                        } else if (obj instanceof Collection) {
/* 59 */                    Iterator it = ((Collection) obj).iterator();
/* 67 */                    while (it.hasNext()) {
/* 73 */                        I00000oIO(ilIO00o1, it.next(), false);
                            }
                        } else if (obj instanceof Map) {
/* 87 */                    Iterator it2 = ((Map) obj).entrySet().iterator();
/* 95 */                    while (it2.hasNext()) {
/* 105 */                       I0000oI00(I000OOo1O, ilIO00o1, (Map.Entry) it2.next(), false);
                            }
                        } else if (obj instanceof Double) {
/* 116 */                   double dDoubleValue = ((Double) obj).doubleValue();
/* 120 */                   if (!z || dDoubleValue != 0.0d) {
/* 137 */                       I0001Ioi1lo((I0000O(ilIO00o1) << 3) | 1);
/* 162 */                       this.I00000oIO.write(ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN).putDouble(dDoubleValue).array());
/* 165 */                       return this;
                            }
                        } else if (obj instanceof Float) {
/* 172 */                   float fFloatValue = ((Float) obj).floatValue();
/* 176 */                   if (!z || fFloatValue != 0.0f) {
/* 192 */                       I0001Ioi1lo((I0000O(ilIO00o1) << 3) | 5);
/* 216 */                       this.I00000oIO.write(ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putFloat(fFloatValue).array());
/* 219 */                       return this;
                            }
                        } else {
/* 222 */                   if (obj instanceof Number) {
/* 230 */                       I0000Il00O(ilIO00o1, ((Number) obj).longValue(), z);
/* 233 */                       return this;
                            }
/* 236 */                   if (obj instanceof Boolean) {
/* 244 */                       I00000oOI(ilIO00o1, ((Boolean) obj).booleanValue() ? 1 : 0, z);
/* 247 */                       return this;
                            }
/* 250 */                   if (!(obj instanceof byte[])) {
/* 291 */                       OIOi0lOI oIOi0lOI = (OIOi0lOI) this.I00000oOI.get(obj.getClass());
/* 293 */                       if (oIOi0lOI != null) {
/* 295 */                           I0000oI00(oIOi0lOI, ilIO00o1, obj, z);
/* 298 */                           return this;
                                }
/* 309 */                       OoloOoIl ooloOoIl = (OoloOoIl) this.I0000Il00O.get(obj.getClass());
/* 311 */                       if (ooloOoIl != null) {
/* 313 */                           OOiI1O oOiI1O = this.I0000oI00;
/* 315 */                           oOiI1O.I00000oOI = false;
/* 317 */                           oOiI1O.I0000O = ilIO00o1;
/* 319 */                           oOiI1O.I0000Il00O = z;
/* 321 */                           ooloOoIl.encode(obj, oOiI1O);
/* 324 */                           return this;
                                }
/* 327 */                       if (obj instanceof iI1I00liI) {
/* 335 */                           I00000oOI(ilIO00o1, ((iI1I00liI) obj).zza(), true);
/* 338 */                           return this;
                                }
/* 341 */                       if (obj instanceof Enum) {
/* 349 */                           I00000oOI(ilIO00o1, ((Enum) obj).ordinal(), true);
/* 352 */                           return this;
                                }
/* 355 */                       I0000oI00(this.I0000O, ilIO00o1, obj, z);
/* 408 */                       return this;
                            }
/* 252 */                   byte[] bArr = (byte[]) obj;
/* 254 */                   if (!z || bArr.length != 0) {
/* 268 */                       I0001Ioi1lo((I0000O(ilIO00o1) << 3) | 2);
/* 272 */                       I0001Ioi1lo(bArr.length);
/* 277 */                       this.I00000oIO.write(bArr);
/* 280 */                       return this;
                            }
                        }
                    }
/* 259 */           return this;
                }

                public final void I00000oOI(IlIO00o1 ilIO00o1, int i, boolean z) throws IOException {
/* 1 */             if (z && i == 0) {
/* 31 */                return;
                    }
/* 11 */            iIIoOIOO0Io iiiooioo0io = (iIIoOIOO0Io) ilIO00o1.I00000oOI(iIIoOIOO0Io.class);
/* 13 */            if (iiiooioo0io == null) {
/* 101 */               iilo0lOo110.I0000O("Field has no @Protobuf config");
/* 408 */               return;
                    }
/* 19 */            int iOrdinal = iiiooioo0io.zzb().ordinal();
/* 23 */            if (iOrdinal == 0) {
/* 92 */                I0001Ioi1lo(iiiooioo0io.zza() << 3);
/* 95 */                I0001Ioi1lo(i);
                    } else if (iOrdinal == 1) {
/* 74 */                I0001Ioi1lo(iiiooioo0io.zza() << 3);
/* 82 */                I0001Ioi1lo((i + i) ^ (i >> 31));
                    } else {
/* 29 */                if (iOrdinal != 2) {
/* 31 */                    return;
                        }
/* 40 */                I0001Ioi1lo((iiiooioo0io.zza() << 3) | 5);
/* 64 */                this.I00000oIO.write(ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putInt(i).array());
                    }
                }

                public final void I0000Il00O(IlIO00o1 ilIO00o1, long j, boolean z) throws IOException {
/* 1 */             if (z && j == 0) {
/* 35 */                return;
                    }
/* 15 */            iIIoOIOO0Io iiiooioo0io = (iIIoOIOO0Io) ilIO00o1.I00000oOI(iIIoOIOO0Io.class);
/* 17 */            if (iiiooioo0io == null) {
/* 107 */               iilo0lOo110.I0000O("Field has no @Protobuf config");
/* 408 */               return;
                    }
/* 23 */            int iOrdinal = iiiooioo0io.zzb().ordinal();
/* 27 */            if (iOrdinal == 0) {
/* 98 */                I0001Ioi1lo(iiiooioo0io.zza() << 3);
/* 101 */               I000II(j);
                    } else if (iOrdinal == 1) {
/* 78 */                I0001Ioi1lo(iiiooioo0io.zza() << 3);
/* 88 */                I000II((j >> 63) ^ (j + j));
                    } else {
/* 33 */                if (iOrdinal != 2) {
/* 35 */                    return;
                        }
/* 43 */                I0001Ioi1lo((iiiooioo0io.zza() << 3) | 1);
/* 68 */                this.I00000oIO.write(ByteBuffer.allocate(8).order(ByteOrder.LITTLE_ENDIAN).putLong(j).array());
                    }
                }

                public final void I0000oI00(OIOi0lOI oIOi0lOI, IlIO00o1 ilIO00o1, Object obj, boolean z) throws IllegalAccessException, IOException, IllegalArgumentException, InvocationTargetException {
/* 6 */             O0oIolOOo o0oIolOOo = new O0oIolOOo(1);
/* 11 */            o0oIolOOo.I00iiI = 0L;
                    try {
/* 13 */                OutputStream outputStream = this.I00000oIO;
/* 15 */                this.I00000oIO = o0oIolOOo;
                        try {
/* 17 */                    oIOi0lOI.encode(obj, this);
/* 20 */                    this.I00000oIO = outputStream;
/* 22 */                    long j = o0oIolOOo.I00iiI;
/* 24 */                    o0oIolOOo.close();
/* 27 */                    if (z && j == 0) {
/* 33 */                        return;
                            }
/* 42 */                    I0001Ioi1lo((I0000O(ilIO00o1) << 3) | 2);
/* 45 */                    I000II(j);
/* 48 */                    oIOi0lOI.encode(obj, this);
                        } catch (Throwable th) {
/* 55 */                    this.I00000oIO = outputStream;
/* 57 */                    throw th;
                        }
                    } catch (Throwable th2) {
                        try {
/* 58 */                    o0oIolOOo.close();
                        } catch (Throwable th3) {
                            try {
/* 77 */                        Throwable.class.getDeclaredMethod("addSuppressed", Throwable.class).invoke(th2, th3);
                            } catch (Exception unused) {
                            }
                        }
/* 204 */               throw th2;
                    }
                }

                public final void I0001Ioi1lo(int i) throws IOException {
                    while (true) {
/* 1 */                 long j = i & (-128);
/* 8 */                 OutputStream outputStream = this.I00000oIO;
/* 10 */                if (j == 0) {
/* 24 */                    outputStream.write(i & 127);
/* 29 */                    return;
                        } else {
/* 16 */                    outputStream.write((i & 127) | Barcode.FORMAT_ITF);
/* 19 */                    i >>>= 7;
                        }
                    }
                }

                public final void I000II(long j) throws IOException {
                    while (true) {
/* 3 */                 long j2 = (-128) & j;
/* 8 */                 OutputStream outputStream = this.I00000oIO;
/* 10 */                if (j2 == 0) {
/* 26 */                    outputStream.write(((int) j) & 127);
/* 29 */                    return;
                        } else {
/* 17 */                    outputStream.write((((int) j) & 127) | Barcode.FORMAT_ITF);
/* 21 */                    j >>>= 7;
                        }
                    }
                }

                @Override
                public final OIOi0o add(IlIO00o1 ilIO00o1, int i) throws IOException {
/* 2 */             I00000oOI(ilIO00o1, i, true);
/* 37 */            return this;
                }

                @Override
/* 5 */         public final OIOi0o add(IlIO00o1 ilIO00o1, long j) throws IOException {
/* 6 */             I0000Il00O(ilIO00o1, j, true);
                    return this;
                }

                @Override
/* 6 */         public final OIOi0o add(IlIO00o1 ilIO00o1, Object obj) throws IllegalAccessException, IOException, IllegalArgumentException, InvocationTargetException {
/* 7 */             I00000oIO(ilIO00o1, obj, true);
                    return this;
                }
            }
